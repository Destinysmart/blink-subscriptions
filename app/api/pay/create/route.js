...
export async function POST(req) {
  try {
    const { username, tier, sats, cycle, contact } = await req.json();
    ...
    const { subId } = await createPendingSub({ username, contact, tier, sats: amount, cycle, paymentHash, paymentRequest });
