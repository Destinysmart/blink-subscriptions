...
export async function getRecentReceives(apiKey, first = 30) {
  ...
  return edges.map((e) => e.node)
    .filter((n) => n.direction === 'RECEIVE' && n.settlementVia?.__typename === 'SettlementViaIntraLedger')
    .map((n) => ({ sats: Math.abs(Number(n.settlementAmount)), from: n.settlementVia.counterPartyUsername, createdAt: n.createdAt }));
}
