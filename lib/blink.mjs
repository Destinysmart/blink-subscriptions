...
export async function createInvoiceForUsername(username, amountSats, memo) {
  ...
  const invoice = await mintInvoiceForRecipient(                 // no apiKey => public
    { recipientWalletId: wallet.id, amountSats, memo, expiresIn: 60 }, // minutes: a one-hour invoice
  );
...
/**
 * Poll invoice status by payment request via lnInvoicePaymentStatusByPaymentRequest —
 * the exact public query Blink's own paywall relies on. Returns PAID|PENDING|EXPIRED.
 * Unlike the by-hash query, this reflects Blink-to-Blink (intraledger) settlement, and
 * PAID stands even when the preimage is withheld (as it is for intraledger).
 */
export async function invoiceStatusByRequest(paymentRequest) {
  const q = `query($input: LnInvoicePaymentStatusByPaymentRequestInput!) {
    lnInvoicePaymentStatusByPaymentRequest(input: $input) { status paymentPreimage }
  }`;
  const d = await gql(q, { input: { paymentRequest } });
  return d.lnInvoicePaymentStatusByPaymentRequest?.status || 'PENDING';
}
