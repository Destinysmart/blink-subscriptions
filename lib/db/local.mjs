...
import { invoiceStatusByHash, invoiceStatus, receivedByHash, createInvoiceForUsername, whoAmI, getRecentReceives } from '../blink.mjs';
...
if (URL.startsWith('file:')) {
  ...
  ({ createClient } = await import('@libsql/client'));           // native, local dev only
} else {
  ({ createClient } = await import('@libsql/client/web'));       // HTTP client — works on Vercel serverless
}
...
async function seed() {
  ...
  await db.execute({
    sql: 'INSERT OR IGNORE INTO creators (id, username, brand, pitch, tiers_json, payout_username) VALUES (?,?,?,?,?,?)',
...
async function findOrProvision(username) {
  ...
  // INSERT OR IGNORE + re-read handles the race where two concurrent requests both provision.
  const cid = id();
  await db.execute({
    sql: 'INSERT OR IGNORE INTO creators (id, username, brand, pitch, tiers_json) VALUES (?,?,?,?,?)',
  ...
  const again = await db.execute({ sql: 'SELECT * FROM creators WHERE username = ?', args: [username] });
  return again.rows[0];
}
...
async function ownsUsername(username, token) {
  const u = String(username || '').toLowerCase().replace(/^@/, '');
  if (!u || !token) return false;
  const r = await db.execute({ sql: 'SELECT manage_token FROM creators WHERE username = ?', args: [u] });
  const c = r.rows[0];
  return !!(c && c.manage_token && c.manage_token === token);
}

// authed dashboard read: only the verified owner (matching manage_token) gets data
export async function getDashboardIfOwner(username, token) {
  await ensureReady();
  const u = String(username || '').toLowerCase().replace(/^@/, '');
  if (!(await ownsUsername(u, token))) return null;
  return { dashboard: await getDashboard(u), full: await getCreator(u) };
}
...
export async function getDashboard(username) {
  ...
  const pend = await db.execute({ sql: 'SELECT id, creator_id, status, payment_hash, payment_request FROM subscriptions WHERE creator_id = ? AND status = ?', args: [c.id, 'pending'] });
  for (const row of pend.rows) {
    if (!row.payment_hash && !row.payment_request) continue;
    try { await settlePendingSub(row); } catch {}
  }
...
export async function activateSub(subId) {
  ...
  // Only the poll that actually flips pending -> active proceeds. When several
  // pollers fire at once (status route + dashboard self-heal, or a burst of
  // manual checks with the console open), the losers get rowsAffected 0 here and
  // return without writing a duplicate invoice.paid event.
  const res = await db.execute({
    sql: "UPDATE subscriptions SET status = 'active', paid_until = ?, next_due = ? WHERE id = ? AND status != 'active'",
    args: [until, until, subId],
  });
  if (!res.rowsAffected) return { paidUntil: until };
  await db.execute({
    sql: 'INSERT INTO events (id, creator_id, type, who, detail, created_at) VALUES (?,?,?,?,?,?)',
...
/**
 * Webhook path: find the pending sub for an invoice hash and activate it.
 * Idempotent via activateSub's atomic flip, so repeated webhook deliveries are safe.
 * Returns the sub id if found (activated or already active), else null.
 */
export async function markPaidByHash(paymentHash) {
  if (!paymentHash) return null;
  await ensureReady();
  const r = await db.execute({ sql: 'SELECT id, status FROM subscriptions WHERE payment_hash = ? ORDER BY created_at DESC LIMIT 1', args: [paymentHash] });
  const sub = r.rows[0];
  if (!sub) return null;
  if (sub.status !== 'active') await activateSub(sub.id);
  return sub.id;
}

/** Read-only Blink key for a payout username, from env RECEIVE_API_KEYS="user1:key1,user2:key2". */
function receiveKeyFor(username) {
  const raw = process.env.RECEIVE_API_KEYS || '';
  const u = String(username || '').toLowerCase().replace(/^@/, '');
  if (!u) return null;
  for (const pair of raw.split(',')) {
    const idx = pair.indexOf(':');
    if (idx < 0) continue;
    const name = pair.slice(0, idx).trim().toLowerCase().replace(/^@/, '');
    const key = pair.slice(idx + 1).trim();
    if (name === u && key) return key;
  }
  return null;
}

/**
 * Decide if a pending sub is paid and activate it. Two detection paths:
 *   1. keyless status queries (fast, works for external Lightning)
 *   2. reading the recipient account's own receives with a read-only key
 *      (catches Blink-to-Blink, which the keyless queries do not report)
 * Returns { status, paidUntil?, via, byReq, byHash, error? } for logging.
 */
export async function settlePendingSub(sub) {
  if (!sub) return { status: 'PENDING' };
  if (sub.status === 'active') return { status: 'PAID', paidUntil: sub.paid_until, via: 'already' };
  let status = 'PENDING', byReq = '-', byHash = '-', via = null;
  try {
    ({ status, byReq, byHash } = await invoiceStatus({ paymentHash: sub.payment_hash, paymentRequest: sub.payment_request }));
    if (status === 'PAID') via = 'query';
  } catch (e) { byReq = 'ERR:' + e.message; }
  if (status !== 'PAID' && sub.payment_hash) {
    let uname = null;
    try {
      const cr = (await db.execute({ sql: 'SELECT username, payout_username FROM creators WHERE id = ?', args: [sub.creator_id] })).rows[0];
      uname = cr?.payout_username || cr?.username || null;
    } catch {}
    const key = receiveKeyFor(uname);
    if (key) {
      try { if (await receivedByHash(key, sub.payment_hash)) { status = 'PAID'; via = 'receives'; } }
      catch (e) { /* stay pending, query path already logged */ }
    }
  }
  if (status === 'PAID') {
    try { const { paidUntil } = await activateSub(sub.id); return { status: 'PAID', paidUntil, via, byReq, byHash }; }
    catch (e) { return { status: 'PENDING', error: 'activateSub: ' + e.message, via, byReq, byHash }; }
  }
  return { status, via, byReq, byHash };
}

export async function updateCreatorTiers(username, token, tiers) {
  const u = String(username || '').toLowerCase().replace(/^@/, '');
  if (!(await ownsUsername(u, token))) return { ok: false, error: 'unauthorized' };
  const c = await findOrProvision(u);
