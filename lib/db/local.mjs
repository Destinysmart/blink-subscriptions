...
import { invoiceStatusByHash, createInvoiceForUsername } from '../blink.mjs';
...
async function init() {
  ...
  for (const col of ['webhook_url TEXT', 'webhook_secret TEXT', 'verified_at TEXT', 'manage_token TEXT', 'claim_hash TEXT']) {
...
export async function getConnector(username, token) {
  ...
  if (!c) return null;
  return { verified: !!c.verified_at, webhookUrl: c.webhook_url || '', secret: c.webhook_secret || '' };
}
