...
import { invoiceStatusByHash, invoiceStatus, createInvoiceForUsername, whoAmI, getRecentReceives } from '../blink.mjs';
...
export async function getDashboard(username) {
  ...
  const pend = await db.execute({ sql: 'SELECT id, payment_hash, payment_request FROM subscriptions WHERE creator_id = ? AND status = ?', args: [c.id, 'pending'] });
  for (const row of pend.rows) {
    if (!row.payment_hash && !row.payment_request) continue;
    try { if ((await invoiceStatus({ paymentHash: row.payment_hash, paymentRequest: row.payment_request })).status === 'PAID') await activateSub(row.id); } catch {}
  }
