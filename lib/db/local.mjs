...
export async function startOwnershipVerify(username) {
  ...
  const nonce = Math.floor(Math.random() * Math.max(3, Math.min(500, Math.floor(FEE_SATS / 20))));
  const expected = FEE_SATS + nonce; // small nonce vs fee, so the 21-sat test price stays ~21
...
export async function pollOwnershipVerify(claimId) {
  ...
  const started = new Date(claim.started_at).getTime() - 180000;
  const claimant = String(claim.username).toLowerCase();

  // primary match: an internal (Blink-to-Blink) payment FROM the claimed username, >= the fee, since the claim began
  const good = receives.find((x) => x.via === 'intraledger' && x.from && String(x.from).toLowerCase() === claimant && x.sats >= Number(claim.expected_sats) - 1 && x.at >= started);
  if (good) { /* verified below */ }
  else {
    // did the exact amount arrive but over Lightning (no sender)? tell them to send by username
    const lnHit = receives.find((x) => x.via !== 'intraledger' && x.sats === Number(claim.expected_sats) && x.at >= started);
    if (lnHit) return { verified: false, error: 'Payment received, but it settled over Lightning so we cannot read the sender. In your Blink app tap Send, choose the username @circularity, and send ' + claim.expected_sats + ' sats — that settles inside Blink and carries your username.' };
    // did the right person pay but a different-looking amount? still accept if it is them
    const wrongUser = receives.find((x) => x.via === 'intraledger' && x.from && String(x.from).toLowerCase() !== claimant && x.sats === Number(claim.expected_sats) && x.at >= started);
    if (wrongUser) { await db.execute({ sql: "UPDATE claims SET status = 'rejected' WHERE id = ?", args: [claimId] }); return { verified: false, error: 'Paid from @' + wrongUser.from + ', not @' + claim.username + '. Send from the account you are claiming.' }; }
    return { pending: true };
  }
  const cand = good;
