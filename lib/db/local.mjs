...
export async function pollOwnershipVerify(claimId) {
  ...
  // find the incoming payment of our expected amount within the window
  const cand = receives.find((x) => x.sats === Number(claim.expected_sats) && new Date(x.createdAt).getTime() >= started);
  if (!cand) return { pending: true };

  if (cand.via !== 'intraledger' || !cand.from) {
    return { verified: false, error: 'Payment received, but it settled over Lightning so we cannot read the sender. Open your Blink app, tap Send, choose the username @circularity, and send ' + claim.expected_sats + ' sats — that settles inside Blink and carries your username.' };
  }
  if (String(cand.from).toLowerCase() !== String(claim.username).toLowerCase()) {
    await db.execute({ sql: "UPDATE claims SET status = 'rejected' WHERE id = ?", args: [claimId] });
    return { verified: false, error: 'Paid from @' + cand.from + ', not @' + claim.username + '. Pay from the account you are claiming.' };
  }
